...
def getRansacHomography(p1, p2):
    ...
    for cycle in range(0,100000):
        ...
        # calculate homography NOTE: so far i am using cv homography
...
h2 = getHomographyMatrix(images[0], images[1])

myHomography = getRansacHomography(images[0], images[1])
...
img = cv2.imread('D:\Skola\ITT\openpose-master\data\DownwardDog\Processed\\000_rendered.png') 
out = cv2.warpPerspective(img,myHomography,(1920, 1080),flags=cv2.INTER_LINEAR)
